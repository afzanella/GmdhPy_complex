...
modification for complex values: 'Andre Zanella'
...
class BaseSONN(object):
    ...
    def __init__(self, seq_type, ref_functions, criterion_type, feature_names, max_layer_count,
        ...
        self.layer_err = np.array([], dtype="clongdouble")          # array of layer's errors
        self.train_layer_err = np.array([], dtype="clongdouble")    # array of layer's train errors
        self.valid = False
        self.loss = None
    ...
    def _new_layer_with_all_neurons(self):
        ...
        for u1 in range(0, n):
            for u2 in range(u1 + 1, n):
                ...
                # y = 3rd degree polynom with complex conjugate
                if RefFunctionType.rfConjCubic in self.param.ref_function_types:
                    layer.add_neuron(u1,u2,RefFunctionType.rfConjCubic,self.model_class,self.loss)

                # y = 2nd degree polynom with absolute values
                if RefFunctionType.rfModQuad in self.param.ref_function_types:
                    layer.add_neuron(u1,u2,RefFunctionType.rfModQuad,self.model_class,self.loss)

                # y = 3rd degree polynom with absolute values
                if RefFunctionType.rfModCubic in self.param.ref_function_types:
                    layer.add_neuron(u1,u2,RefFunctionType.rfModCubic,self.model_class,self.loss)
    ...
    def _set_internal_data(self, layer, data, x):
        ...
        if layer is None:
            ...
            out_x = np.zeros((data_m, out_size), dtype="clongdouble")
