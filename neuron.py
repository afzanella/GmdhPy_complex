...
def H(X): #novo
    #Calcula a matriz hermetiana (conjugado complexo), para o calculo do LS com valores imaginarios
    return X.conj().T

def lsqr(X,Y): #novo
    #Calcula os coeficientes para cada neuronio utilizando least squares
    Xh = H(X)
    A = np.dot(inv(np.dot(Xh,X)),np.dot(Xh,Y))
    return A

def solve_lsqr(X, y, alpha, max_iter=None, tol=1e-3):
...
class RefFunctionType(Enum):
    ...
    def get_name(cls, value):
        if value == cls.rfUnknown:
            ...
            return 'Harmonic'
        elif value == cls.rfConjCubic:
            return 'ConjCubic'
        elif value == cls.rfModQuad:
            return 'ModQuad'
        elif value == cls.rfModCubic:
            return 'ModCubic'
        else:
            return 'Unknown'
    ...
    def get(arg):
        ...
        if arg == 'linear':
            ...
            return RefFunctionType.rfConjCubic
        elif arg == 'modquad':
            return RefFunctionType.rfModQuad
        elif arg == 'modcubic':
            return RefFunctionType.rfModCubic
        else:
...
class PolynomNeuron(Neuron):
    ...
    def _transfer_conjcubic(self,u1,u2,w):
        return self.activation(w[0]+ w[1]*u1 + w[2]*u2 + w[3]*u1*u1*np.conj(u1)+w[4]*u1*u1*np.conj(u2)+w[5]*u1*u2*np.conj(u1)+w[6]*u1*u2*np.conj(u2)+w[7]*u2*u2*np.conj(u1)+w[8]*u2*u2*np.conj(u2))

    def _transfer_modquad(self,u1,u2,w):
        return self.activation(w[0]+ w[1]*u1 + w[2]*u2 + w[3]*u1*np.abs(u1) + w[4]*u1*np.abs(u2) + w[5]*u2*np.abs(u1) + w[6]*u2*np.abs(u2))

    def _transfer_modcubic(self,u1,u2,w):
        return self.activation(w[0]+ w[1]*u1 + w[2]*u2 + w[3]*u1*np.abs(u1) + w[4]*u1*np.abs(u2) + w[5]*u2*np.abs(u1) + w[6]*u2*np.abs(u2) + w[7]*u1*u1*np.abs(u1) + w[8]*u1*u1*np.abs(u2) + w[9]*u1*u2*np.abs(u1) + w[10]*u1*u2*np.abs(u2) + w[11]*u2*u2*np.abs(u1) + w[12]*u2*u2*np.abs(u2))
    ...
    def set_type(self, new_type):
        ...
        if new_type == RefFunctionType.rfLinear:
            ...
            self.fw_size = 10
        elif new_type == RefFunctionType.rfConjCubic:
            self.transfer = self._transfer_conjcubic
            self.fw_size = 9
        elif new_type == RefFunctionType.rfModQuad:
            self.transfer = self._transfer_modquad
            self.fw_size = 7
        elif new_type == RefFunctionType.rfModCubic:
            self.transfer = self._transfer_modcubic
            self.fw_size = 13
        else:
    ...
    def get_name(self):
        ...
        elif self.ftype == RefFunctionType.rfConjCubic:
            return '3rd degree polynom with complex conjugate'
        elif self.ftype == RefFunctionType.rfModQuad:
            return '2nd degree polynom with absolute values'
        elif self.ftype == RefFunctionType.rfModCubic:
            return '3rd degree polynom with absolute values'
        else:
            return 'Unknown'
    ...
    def get_polynom_inputs(self, ftype, u1_index, u2_index, source):
        ...
        a = np.empty((source.shape[0], self.fw_size), dtype="clongdouble")
        ...
        if RefFunctionType.rfConjCubic == ftype:
            a[:, 3] = u1x * u1x * np.conj(u1x)
            a[:, 4] = u1x * u1x * np.conj(u2x)
            a[:, 5] = u1x * u2x * np.conj(u1x)
            a[:, 6] = u1x * u2x * np.conj(u2x)
            a[:, 7] = u2x * u2x * np.conj(u1x)
            a[:, 8] = u2x * u2x * np.conj(u2x)

        if RefFunctionType.rfModQuad == ftype:
            a[:, 3] = u1x*np.abs(u1x)
            a[:, 4] = u1x*np.abs(u2x)
            a[:, 5] = u2x*np.abs(u1x)
            a[:, 6] = u2x*np.abs(u2x)

        if RefFunctionType.rfModCubic == ftype:
            a[:, 3]  = u1x*np.abs(u1x)
            a[:, 4]  = u1x*np.abs(u2x)
            a[:, 5]  = u2x*np.abs(u1x)
            a[:, 6]  = u2x*np.abs(u2x)
            a[:, 7]  = u1x*u1x*np.abs(u1x)
            a[:, 8]  = u1x*u1x*np.abs(u2x)
            a[:, 9]  = u1x*u2x*np.abs(u1x)
            a[:, 10] = u1x*u2x*np.abs(u2x)
            a[:, 11] = u2x*u2x*np.abs(u1x)
            a[:, 12] = u2x*u2x*np.abs(u2x)

        return a

    #new function for complex data
    def _fit_regressor(self, x, y, params):
        ...
        a2 = a[:, 1:]
        first = np.repeat(1,np.shape(a2)[0]).reshape([np.shape(a2)[0],1])
        a2 = np.hstack((first,a2))
        # reg.fit(a2, y)
        coef,res,rnk,s = lstsq(a2,y)
        w = coef
        return w

    # #Original function, only works with real data
    # def _fit_regressor(self, x, y, params):
    #     a = self.get_polynom_inputs(self.ftype, self.u1_index, self.u2_index, x)
    #     reg = linear_model.Ridge(alpha=params['l2'], solver='lsqr')
    #     a2 = a[:, 1:]
